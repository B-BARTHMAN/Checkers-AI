...
MCSearchResult MCSearch(const Position& pos, float (*evaluate)(const Position&), int threadID) {
	...
	static MCNode nodes[THREAD_COUNT][ITER_COUNT];
	int nodeCount = 0;

	nodes[threadID][nodeCount++].reset();
	nodes[threadID][0].position = pos;
	...
	while (true) {
		...
		node = &nodes[threadID][0]; // starting position
		...
		while (true) {
			...
			if (action.nextState == nullptr) {
				action.nextState = &nodes[threadID][nodeCount]; //set the next state of the action
				nodes[threadID][nodeCount].reset();
				nodes[threadID][nodeCount].position = node->position.doMove(action.move); //set the new position
				node = &nodes[threadID][nodeCount++]; //this is the selected node
			}
	...
	MCSearchResult res;
	res.outcome = nodes[threadID][0].q;

	for (ActionMove& move : nodes[threadID][0].actions) {
...
MCSearchResult MCSearchSpec(const Position& pos, float(*evaluate)(const Position&, int), int parameter, int threadID)
{
	...
	while (true) {
		...
		if (terminal)
			...
			value = evaluate(node->position, parameter);
